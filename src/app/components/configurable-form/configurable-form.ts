...
import { CustomUpload } from '../custom-upload/custom-upload';
@Component({
...
export class ConfigurableForm implements OnInit {
    ...
    if (control && control.invalid && (control.dirty || control.touched)) {
      ...
      if (errors) {
        ...
        return this.errorMessages[errorKey]
          ? `${fieldLabel} : ${this.errorMessages[errorKey](errors[errorKey])}`
  ...
  getGroupValidationErrors(groupId: string): string[] {
    ...
    Object.keys(groupForm.errors).forEach((errorKey) => {
      const errorMessage = this.errorMessages[errorKey]
        ? this.errorMessages[errorKey](groupForm.errors![errorKey])
        : `Erreur de validation du groupe: ${errorKey}`;
  ...
  getGlobalValidationErrors(): string[] {
    ...
    Object.keys(form.errors).forEach((errorKey) => {
      const errorMessage = this.errorMessages[errorKey]
        ? this.errorMessages[errorKey](form.errors![errorKey])
        : `Erreur de validation globale: ${errorKey}`;
      errors.push(errorMessage);
    });
  ...
  getDirectFieldError(fieldName: string): string | null {
    ...
    if (control && control.invalid && (control.dirty || control.touched)) {
      const errors = control.errors;
      if (errors) {
        const fieldLabel = this.structure()?.formFields?.find(
  ...
  isFormFieldGroup(
    element: FormField<any> | FormFieldGroup
  ): element is FormFieldGroup {
    return (
      'fields' in element && Array.isArray((element as FormFieldGroup).fields)
    );
  }
  ...
  trackByElementId(
    ...
    return this.isFormField(element)
      ? element.id
      : (element as FormFieldGroup).id;
  }
