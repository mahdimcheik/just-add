...
export const errorMessages: { [key: string]: (errValue: any) => string } = {
  ...
  minlength: (err) =>
    `Ce champ doit contenir au moins ${err.requiredLength} caractères.`,
  maxlength: (err) =>
    `Ce champ ne doit pas dépasser ${err.requiredLength} caractères.`,
  ...
  invalidFrenchPostalCode: () =>
    'Le code postal français doit contenir 5 chiffres.',
  invalidBelgiumPostalCode: () =>
    'Le code postal belge doit contenir 4 chiffres.',
  contactEmailRequired: () =>
    'Un email de contact est requis si vous souhaitez recevoir la newsletter.',

  // Generic group validation error
  groupValidationError: () =>
    "Une erreur de validation s'est produite dans cette section.",
};
